...
if packageIdentifier is "" then
	...
	set result to display alert "Uninstall RethinkDB?" message "This will uninstall the RethinkDB binaries and support files from: " & displayPath & return & return & "Your data files will not be affected. You will be asked for your Administrator password." as warning buttons {"Uninstall", "Cancel"} default button "Cancel"
	
	-- run the uninstall
	if the button returned of result is "Uninstall" then
		do shell script "set -e; /usr/sbin/pkgutil --files '" & packageIdentifier & "' | sort -r | egrep -vx 'usr|usr/local|usr/local/[^/]*' | while read FILE; do rm -fd \"" & installedPath & "$FILE\"; done && pkgutil --forget '" & packageIdentifier & "'" with administrator privileges
		display alert "RethinkDB sucessfully removed from:" & return & installedPath giving up after 10
	end if
end if