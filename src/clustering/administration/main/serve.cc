...
#include "clustering/administration/main/swap_checker.hpp"
                              ...
                              serve_info.reql_http_proxy);
        {
...

            {
...

                {
                    ...
                    /* `swap_checker` periodically checks to see if we are using swap
                    memory, and will log a warning. */

                    scoped_ptr_t<swap_checker_t> swap_checker;
                    if (i_am_a_server) {
                        swap_checker.init(new swap_checker_t(&rdb_ctx));
                    }
