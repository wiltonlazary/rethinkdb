...
public:
    ...
    bool contains(const store_key_t& key) const {
        rassert(left <= right);
        bool left_ok = left <= key;
        bool right_ok = key < right;
        return left_ok && right_ok;
    }

    bool contains(const uint8_t *key, uint8_t size) const {
        rassert(left <= right);
        bool left_ok = sized_strcmp(left.contents(), left.size(), key, size) <= 0;
        bool right_ok = sized_strcmp(key, size, right.contents(), right.size()) < 0;
        return left_ok && right_ok;
    }

    bool contains(const btree_key_t *key) const {
        return contains(key->contents, key->size);
    }
