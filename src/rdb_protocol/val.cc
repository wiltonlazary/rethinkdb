...
datum_t table_t::batched_insert(
    ...
    std::map<datum_t, int> pkey_to_pos;
    if (return_changes == return_changes_t::ALWAYS) {
        // Create map of primary keys to position in insert command.
        for (size_t i = 0; i < valid_inserts.size(); ++i) {
            pkey_to_pos[valid_inserts[i].get_field(get_pkey().c_str())] = i;
        }
    }
    ...
    if (return_changes == return_changes_t::ALWAYS) {
        // Changes returned in insert_stats are unordered due to sharding.
        // Reorder them by the map created above to enforce ordering.

        std::vector<ql::datum_t> new_changes{pkey_to_pos.size()};
        ql::datum_t changes = std::move(insert_stats.get_field("changes"));

        for (size_t i = 0; i < changes.arr_size(); ++i) {
            ql::datum_t change = std::move(changes.get(i));
            int pos = pkey_to_pos[
                change.get_field("new_val").get_field(get_pkey().c_str())];
            new_changes.at(pos) = std::move(change);
        }

        ql::datum_t new_changes_datum = ql::datum_t(
            std::move(new_changes),
            env->limits());
        ql::datum_t new_stats_datum = ql::datum_t(
            std::map<datum_string_t, ql::datum_t>{
                std::pair<datum_string_t, ql::datum_t>{
                    ...
                    new_changes_datum}});
        insert_stats = insert_stats.merge(new_stats_datum);
    }
